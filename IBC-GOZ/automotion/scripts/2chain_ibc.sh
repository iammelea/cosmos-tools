...
echo "Hello,"
sleep 1
echo "So, let's build our next network to connect them via IBC"
sleep 1
...
read password
echo "Enter Token name example: aetc ok, AETC NO ok"
read DENOM
echo "Enter 2 Token name example: abtg ok, ABTG NO ok"
read DENOM2
echo "Enter chain-id example: meleo-22"
read CHAINID
echo "Enter Domain name example: ibc.domain.com, if no domain localhost"
read DOMAIN
echo "Enter node name aka moniker: example validator2"
read MONIKER
echo "Enter RPC port example 36658:"
read RPC
echo "Enter P2P port example 36655:"
read P2P
echo "Enter faucet port  example 8001:"
read FCT
echo "Enter dir name for gaia 2 chain example .2ibcgaia:"
read DIR
...
echo "Now write this mnemonic phrase in a safe place."
sleep 2
